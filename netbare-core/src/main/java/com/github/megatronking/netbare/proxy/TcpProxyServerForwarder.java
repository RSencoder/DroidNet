...
public final class TcpProxyServerForwarder implements ProxyServerForwarder {
    ...
    public void forward(byte[] packet, int len, OutputStream output) {
        ...
        if (localPort != mProxyServer.getPort()) {  //这个if-else语句与抓包精灵博客写的步骤3、15相同
            ...
            ipHeader.setSourceIp(remoteIp);  //源ip改为目标ip
            ipHeader.setDestinationIp(mProxyServer.getIp());  //目标ip改为代理服务器ip
            tcpHeader.setDestinationPort(mProxyServer.getPort());  //目标端口改为代理服务器端口
